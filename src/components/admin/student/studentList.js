...
function StudentList(props) {
    ...
    const { yearOfAdmission, program, branch, refetch, setRegisterStudent, setEdit, setRefetch } = props;
    useEffect(() => {
        const params = {
            yearOfAdmission,
            program,
        ...
        axios.get('/api/admin/student', { params }).then(res => {
            // sort by MIS
    ...
    useEffect(() => {
        if (search !== "") {
            const fStudents = students.filter(student => {
                return `${student.name} ${student.MIS} ${student.email}`.toLowerCase().includes(search.toLowerCase())
            })
    ...
    function deleteStudent(MIS) {
        axios.delete(`/api/admin/student/${MIS}`).then(res => {
            setRefetch(!refetch);
        }).catch(err => {
            console.log(err)
        })
